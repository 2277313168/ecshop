...
<div class="main-div">
  <form action="/shop/ecshop/index.php/Admin/Admin/adminEdit" method="post" name="theForm" enctype="multipart/form-data"
    ...
    <table width="100%" id="general-table">
      ...
      <tr>
        ...
        <td><input type="text" name="admin_name" maxlength="20" value="<?php echo ($admin["admin_name"]); ?>" size="27"> <font color="red">*</font>
        </td>
      </tr>
      ...
      <tr>
        ...
        <td>
          <?php if(is_array($roleList)): $i = 0; $__LIST__ = $roleList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><input  type="checkbox"  name="roleIds[]"  value="<?php echo ($vo["role_id"]); ?>"
            <?php if(strpos(','.$admin['role_ids'].',' , ','. $vo['role_id'] .',') !==FALSE): ?>checked="checked"<?php endif; ?> /><?php echo ($vo['role_name']); ?><br/><?php endforeach; endif; else: echo "" ;endif; ?>
        </td>
      </tr>

      <?php if($admin['admin_id'] > 1): ?><tr>
        <td class="label">是否启用：</td>
        <td>
          <input type="radio" name="is_use" value="1"
                 <?php if($admin['is_use'] == 1): ?>checked="checked"<?php endif; ?>/>启用
          <input type="radio" name="is_use" value="0"
          <?php if($admin['is_use'] == 0): ?>checked="checked"<?php endif; ?> />禁用
        </td>
      </tr><?php endif; ?>
